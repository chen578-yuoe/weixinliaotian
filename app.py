...
ai_provider = st.sidebar.selectbox("选择 AI 模型", ["DeepSeek", "豆包 (Doubao)"])

if ai_provider == "DeepSeek":
    api_key = st.sidebar.text_input("DeepSeek API Key", type="password", placeholder="sk-...")
    st.sidebar.markdown("[👉 获取 DeepSeek API Key](https://platform.deepseek.com/)")
    base_url = "https://api.deepseek.com"
    model_name = "deepseek-chat"
else:
    api_key = st.sidebar.text_input("火山引擎(豆包) API Key", type="password", placeholder="...")
    model_name = st.sidebar.text_input("推理接入点 (Endpoint ID)", placeholder="ep-...")
    st.sidebar.markdown("[👉 获取豆包 API Key 及接入点](https://console.volcengine.com/ark/region:ark+cn-beijing/endpoint)")
    base_url = "https://ark.cn-beijing.volces.com/api/v3"

st.sidebar.header("🛠️ 分析设置")
max_lines = st.sidebar.slider("AI分析抽取行数", min_value=1000, max_value=2000, value=1000, step=100)
st.sidebar.markdown("💡 *注：统计图表会使用**全量数据**，仅在发给 AI 分析时进行随机抽取以节省费用。*")
...
with col_right:
    ...
    if st.button("🚀 开始分析", type="primary", use_container_width=True):
        ...
        if not api_key:
            ...
            st.stop()
        if ai_provider == "豆包 (Doubao)" and not model_name:
            st.error("⚠️ 使用豆包模型时，必须填写推理接入点 (Endpoint ID)！")
            st.stop()
        ...
        try:
            ...
            if len(sorted_users) > 0:
                ...
                with col_c1:
                    st.markdown("##### 🏆 谁是话痨？(全量消息数对比)")
                    st.bar_chart(df.set_index("用户")["消息数量"], color="#FF4B4B")
                with col_c2:
                    st.markdown("##### ✍️ 谁爱写小作文？(全量总字数对比)")
            ...
            st.success(f"✅ 成功读取文件！共发现 {len(messages)} 条记录。统计图表已计算全量数据，为节约成本，已随机抽取 {len(selected_msgs)} 条发送给 AI 进行情感分析。")
